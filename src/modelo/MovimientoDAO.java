...
public class MovimientoDAO extends ConexionPSQL {
    ...
    public String obtnerNextCode() throws SQLException {
        int max = 0;
        abrirConexion();
        sentencia = conexion.prepareStatement("select max(codmovimiento) from movimientos where codtipomovimiento='EPV'");
        resultado = sentencia.executeQuery();
        if (resultado.next()) {
    ...
    public DataSet obtenerVenta(String identificacion) throws SQLException {
        DataSet dt = new DataSet();
        abrirConexion();
        sentencia = conexion.prepareStatement("select codmovimiento, to_char(fechamovimiento,'dd/mm/yyyy') as fecha,concepto,estado from movimientos "
